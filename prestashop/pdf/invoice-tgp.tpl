...
<table style="width: 100%">
	<tr>
        ...
        <td style="width: 100%;vertical-align: top; font-size: 7pt;">
                        ...
                        <table style="width: 100%;">
                            {foreach from=$order_invoice->getOrderPaymentCollection() item=payment}
                                <tr>
                                    ...
                                    <td style="width: 50%; display: none;">{displayPrice price=$payment->amount currency=$order->id_currency}</td>
                                </tr>
                                {foreachelse}
                                <tr>
		...
		<td style="width: 100%; text-align: right">
			<table style="width: 100%; font-size: 8pt;">
				<tr style="line-height:4px;">
					<td style="text-align: left; background-color: #4D4D4D; color: #FFF; padding-left: 10px; font-weight: bold; width: {if !$tax_excluded_display}35%{else}45%{/if}">{l s='Product' pdf='true'}</td>
					<!-- unit price tax excluded is mandatory -->
					{if !$tax_excluded_display}
						<td style="background-color: #4D4D4D; color: #FFF; text-align: right; font-weight: bold; width: 20%">{l s='Unit Price' pdf='true'}</td>
					{/if}
					<td style="background-color: #4D4D4D; color: #FFF; text-align: right; font-weight: bold; width: 10%; display: none">
				...
				<tr style="line-height:6px;background-color:{$bgcolor};">
                    ...
                    <td style="text-align: right; width: 20%; white-space: nowrap;">
                        {displayPrice currency=$order->id_currency price=$order_detail.price}
                    </td>
					{*if !$tax_excluded_display}
						<td style="text-align: right; width: 20%; white-space: nowrap;">
					...
					{/if}
					<td style="text-align: right; width: 10%; white-space: nowrap; display: none;">
					{if $tax_excluded_display}
						{displayPrice currency=$order->id_currency price=$order_detail.unit_price_tax_excl}
					{else}
					...
					{if (isset($order_detail.reduction_amount) && $order_detail.reduction_amount > 0)}
                        ...
                        {$total_discount = $total_discount + $order_detail.reduction_amount}
					{elseif (isset($order_detail.reduction_percent) && $order_detail.reduction_percent > 0)}
						-{$order_detail.reduction_percent}%
                        {$total_discount = $total_discount + ($order_detail.price * $order_detail.reduction_percent / 100)}
					{else}
					--
					{/if}
					</td>
				...
				{foreach $cart_rules as $cart_rule}
					...
					<tr style="line-height:6px;background-color:{$bgcolor};text-align:left;">
						<td style="line-height:6px;text-align:left;width:60%;vertical-align:top" colspan="{if !$tax_excluded_display}5{else}4{/if}">{l s='Cart Rule' pdf='true'}: {$cart_rule.name}</td>
						<td>
							{if $tax_excluded_display}
            ...
            {assign var="total_paid_tax" value=$order_invoice->total_paid_tax_incl - $order_invoice->total_paid_tax_excl}
            {assign var="total_shipping_tax" value=$order_invoice->total_shipping_tax_incl-$order_invoice->total_shipping_tax_excl}
            {assign var="total_products_tax" value=$order_invoice->total_paid_tax_incl - $order_invoice->total_shipping_tax_incl + $order_invoice->total_discount_tax_incl - $order_invoice->total_products}
            ...
            <table style="width: 100%">
				{if (($order_invoice->total_paid_tax_incl - $order_invoice->total_paid_tax_excl) > 0)}
                    <tr style="line-height:5px;">
                        ...
                        <td style="width: 17%; text-align: right;">{displayPrice currency=$order->id_currency price=$order_invoice->total_products + $total_discount}</td>
                    </tr>
                    {if (isset($total_discount) && $total_discount > 0)}
                    <tr>
                        <td style="text-align: right; font-weight: bold">{l s="Total discount" pdf='true'}</td>
                        <td>- {displayPrice currency=$order->id_currency price=$total_discount}</td>
                    </tr>
                    {/if}
                    {if ($order_invoice->total_paid_tax_incl - $order_invoice->total_paid_tax_excl) > 0}
                        <tr style="line-height:5px;">
                            <td style="text-align: right; font-weight: bold">{l s='Total Tax' pdf='true'}</td>
                            <td style="width: 17%; text-align: right;">{displayPrice currency=$order->id_currency price=$total_products_tax}</td>
                        </tr>
                    {/if}
                    <!--tr style="line-height:5px;">
				...
				{if $order_invoice->total_shipping_tax_incl > 0}
                    ...
                    <tr>
                        ...
                        <td>{displayPrice currency=$order->id_currency price=$total_shipping_tax}</td>
                    </tr>
				{/if}
...
{if $order_invoice->total_paid_tax_incl - $order_invoice->total_paid_tax_excl == 0}
    <span>{l s='Art. 21 * Operación exenta según Artículo 21.1 de la Ley 37/1992, de 28 de Diciembre, del Impuesto Sobre el Valor Añadido.' pdf='true'}</span>
{/if}
